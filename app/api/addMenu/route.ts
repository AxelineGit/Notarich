...
export async function POST(req: NextRequest) {
  try {
    ...
    const [existing] = await db.query('SELECT id FROM menu WHERE name = ?', [name]);
    if ((existing as any[]).length > 0) {
      return NextResponse.json({ message: 'Menu name already exists' }, { status: 400 });
    }
    ...
    if (ingredients.length > 0) {
      const ingredientValues = ingredients
        .map((ing: any) => `(${menuId}, ${ing.ingredientId}, ${ing.amount})`)
        .join(",");
      await db.query(`INSERT INTO menuIngredient (menuId, ingredientId, amount) VALUES ${ingredientValues}`);
    }

    if (modifierIds.length > 0) {
      const modifierValues = modifierIds.map((modId: number) => `(${menuId}, ${modId})`).join(",");
      await db.query(`INSERT INTO menuModifier (menuId, modifierId) VALUES ${modifierValues}`);
    }

    if (discountId) {
      await db.query(`INSERT INTO menuDiscount (menuId, discountId) VALUES (?, ?)`, [menuId, discountId]);
    }

    const [ingredientsData]: any = await db.query(
      `SELECT i.stock, mi.amount FROM menuIngredient mi JOIN ingredient i ON mi.ingredientId = i.id WHERE mi.menuId = ?`,
      [menuId]
    );
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (!id || !name || isNaN(price)) {
      return NextResponse.json({ message: "ID, Name, dan Price wajib diisi" }, { status: 400 });
    }

    if (name) {
      const [existingRows]: any = await db.query('SELECT id FROM menu WHERE name = ? AND id != ?', [name, id]);
      if (existingRows.length > 0) {
        return NextResponse.json({ message: 'Menu name already exists' }, { status: 409 });
      }
    }
    ...
    values.push(id);
    await db.query(`UPDATE menu SET ${updateFields.join(", ")} WHERE id = ?`, values);

    await db.query(`DELETE FROM menuIngredient WHERE menuId = ?`, [id]);
    if (ingredients.length > 0) {
      const ingredientValues = ingredients.map((ing: any) => `(${id}, ${ing.ingredientId}, ${ing.amount})`).join(",");
      await db.query(`INSERT INTO menuIngredient (menuId, ingredientId, amount) VALUES ${ingredientValues}`);
    }

    await db.query(`DELETE FROM menuModifier WHERE menuId = ?`, [id]);
    if (modifierIds.length > 0) {
      const modifierValues = modifierIds.map((modId: number) => `(${id}, ${modId})`).join(",");
      await db.query(`INSERT INTO menuModifier (menuId, modifierId) VALUES ${modifierValues}`);
    }

    await db.query(`DELETE FROM menuDiscount WHERE menuId = ?`, [id]);
    if (discountId) {
      await db.query(`INSERT INTO menuDiscount (menuId, discountId) VALUES (?, ?)`, [id, discountId]);
    }
